...
-- 目标:
--   1. 给 exam_questions.question_uid 做回填规则 (与 parse-questions-v4.js generateQuestionUID 对齐)
--   2. 给 question_types 表补齐 multi_choice + 综合/solve/essay 等别名映射
--   3. 加一个 question_type_audit 表, 记录枚举回退与人工修正历史
-- =============================================================================
...
-- 用 ON CONFLICT DO NOTHING 防重: 同 (paper_id, question_number) 可能已存在,
-- 此时保留原行不动, 只补缺的.
...
-- 1.3 用 paper_id + question_number 做兜底 (少数 subject/year 缺失的题)
UPDATE exam_questions eq
...
-- 2. 补齐 question_types 中缺失的枚举别名
--    解析层 prompt 只允许 choice/multi_choice/fill/solve 四种, 与字典 17 种不对齐.
...
-- 3. question_type_audit: 记录每次入库校验的 fallback
