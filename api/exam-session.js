...
export async function startExamSession(req, res) {
  ...
  try {
    let query = `
      SELECT eq.*, ep.year, ep.subject, ep.province_code, p.name as province_name
...
export async function submitExamSession(req, res) {
  ...
  try {
    const sessions = await db.all(
      'SELECT * FROM exam_sessions WHERE id = ? AND user_email = ? AND status = ?',
      [sessionId, email, 'active']
    );
    ...
    for (const answer of answers) {
      ...
      const isCorrect = question.answer && answer.answer &&
        question.answer.trim().toLowerCase() === answer.answer.trim().toLowerCase();
      ...
      if (!isCorrect) {
        const existingWrong = await db.all(
          'SELECT id FROM wrong_questions WHERE user_email = ? AND json_extract(data, "$.question_id") = ?',
          [email, answer.questionId]
        );
...
export async function getExamHistory(req, res) {
  ...
  try {
    ...
    const total = await db.all(
      'SELECT COUNT(*) as count FROM exam_sessions WHERE user_email = ?',
      [email]
    );
