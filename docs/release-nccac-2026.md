...
- 比赛版基线，**只收 bug fix 与发布基建/门禁改动**（后者不改运行时行为）；功能演进一律留在 `main`。

## 小 bug 标准流程

1. **main 先改**：在 `main` 修复并跑全量门禁（`npm test` + `npm run gate`，13 段全绿）。
...
- 发布基建/门禁类改动（**不改运行时行为**）也可 cherry-pick 进 `release/nccac-2026` 并递增 tag；运行时功能改动仍不得进 release。

## 版本记录

- `nccac-2026.2`：新增第 13 段契约门禁（essay/analyze 前端 payload 键 ⊆ 后端 Zod schema），**无运行时行为变更**。
