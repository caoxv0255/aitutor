...
| # | 缺口 | 证据 | 影响 page | 优先级 |
|---|---|---|---|---|
| **G13** | **`raw_text` 里的公式没有 `$` 定界符，只有 `latex_formulas` 有**：`api/routes/vision-parse.js` 的 prompt 只要求 **latex_formulas** 用 `$...$` / `$$...$$` 包裹，`raw_text` 里仍是 `x^2 + 2x - 3 = 0` 这种纯文本形态。前端已能渲染 `$...$`（一旦后端给了就会渲染），但题干正文里的公式目前仍是文本 | 实测 2026-09-22 `/api/vision/batch-parse`：`raw_text: "Solve the equation x^2 + 2x - 3 = 0…"`（无 `$`），`latex_formulas: ["$x^2 + 2x - 3 = 0$", "$S = \\pi \\cdot r^2$", "$(a+b)^2 - (a-b)^2$"]` | photo-solve | **P2**：建议后端在 `raw_text` 中也用 `$` 包裹公式（改 prompt 一行），前端无需再改；在此之前题干公式显示为文本，公式区单独渲染 |
| **G12** | **原型里的"相似题推荐 / 置信度 / 分步解析时间线"没有后端支撑**：`pwa-photo.html`、`vision-result.html`（2026-09-22 已删）里的相似题卡（匹配度 % / 难度 / 年份）、"置信度 98%"、"耗时 2.4s / Qwen-VL-Max"、"已被 12 个同学标记为正确"**全是硬编码假数据** —— 两页源码零 `fetch`。后端候选接口存在（`POST /api/graphrag/similar-questions` 转发 GraphRAG；`rag-search` 有 similarity 检索），但可用性未实测、返回字段未确认 | 证据：两页源码 0 处 fetch / XHR；`api/routes/graphrag.js:136`、`api/routes/rag-search.js:229` | photo-solve（收编时**未**吸收） | **P2**：先实测 graphrag 是否活着、返回什么字段，再决定接不接；在此之前宁可不放这一块，也不复制假数据 |
| **G11** | **essay_reports schema drift（第三例）+ 成功判定错误 + reportId 丢失**：线上表是 F3 旧 schema（data jsonb/images/title/score），代码按 db.js 新 schema 写（report_id/transcript/annotations/meta）→ INSERT 必然失败；且 essayService 成功判定 body.success 恒 falsy（proxy 返回原生 OpenAI 格式）→ LLM 成功也报 LLM_ERROR。**E2E 实测（2026-09-22）**：三层全修后真实 LLM 出报告并持久化（reportId er_mubyzrk5_0hspw84p）；遗留：LLM 输出 JSON 解析为空（scores/summary 空，prompt/解析鲁棒性问题，随 G7 一起处理） | essay | ✅ 管道已通（迁移 038 + 判定修复 + reportId 回传）；评分内容质量待 G7 |
