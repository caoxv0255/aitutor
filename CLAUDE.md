...
admin 用户硬编码在 `api/register.js` 中（邮箱 `admin@uibe.edu.cn`），密码通过 bcrypt 哈希存储。

## 架构概览

```
server.js                  # Express 入口，路由注册、静态文件、中间件
├── api/                   # 后端 API 处理函数（每个文件一个 handler）
│   ├── db.js              # PostgreSQL 连接池 + 自动建表（pg 库）
│   ├── auth.js            # JWT 认证中间件
│   ├── login.js           # 登录
│   ├── register.js        # 注册（含 admin 特殊逻辑）
│   ├── questions.js       # 错题 CRUD（JSONB 存储）
│   ├── reports.js         # 诊断报告 CRUD
│   ├── generate-paper.js  # 个性化试卷生成（高考+中考九科）
│   ├── explain-question.js # AI 题目讲解（调用 LLM）
│   ├── learning-path.js   # 学习路径规划（高考+中考九科）
│   ├── knowledge-points.js # 知识点查询 + 薄弱点加权分析
│   ├── tasks.js           # 异步任务管理
...
│   ├── index.html         # 首页（高考+中考入口）
│   ├── style.css          # 全站共享样式（深色主题 v4）
│   ├── brand.css          # 跨端品牌一致性变量
│   ├── components.js      # 统一导航/页脚/QR码组件
│   ├── qr.js              # QR码生成器（纯 Canvas，无依赖）
│   ├── theme-utils.js     # 明暗主题切换
│   ├── zhongkao.html      # 中考专区首页（九科）
│   ├── zhongkao-policy.html # 中考政策解读
│   ├── wrong-book.html    # 错题本PC端查看
│   ├── dashboard.html     # 学生个人中心（含统计+QR）
│   ├── login.html / register.html
│   ├── {subject}-exam.html    # 各科模拟试卷页
│   ├── {subject}-report.html  # 各科趋势分析报告页
│   ├── personalized-paper.html # 个性化试卷
│   ├── learning-path.html      # 学习路径
│   ├── my-reports.html / my-weak-points.html
│   ├── question-explainer.html  # AI 讲题
│   ├── 2026-policy.html         # 高考政策解读
│   └── methodology.html         # 预测方法论
├── public/                # PWA 手机端（拍照搜题主入口）
│   ├── index.html         # 手机端首页（拍照搜题）
│   ├── manifest.json      # PWA 配置
│   ├── sw.js              # Service Worker（v8）
│   └── src/               # 手机端 JS 模块
├── database/              # 题库数据
│   ├── seed_knowledge_points.json       # 高考知识点（47个，6科）
│   ├── seed_knowledge_points_zhongkao.json # 中考知识点（42个，9科）
│   └── 北京中考真题（25）/              # 中考真题目录
├── uploads/               # 用户上传的图片存储
└── deploy/
    ├── uibe.conf          # Nginx 配置
    ├── uibe-tutor.service # systemd 服务定义
    └── setup.sh           # 一键部署脚本
```
...
使用 PostgreSQL，通过 `api/db.js` 自动建表（无迁移工具）。核心表：
...
`api/knowledge-points.js` 的薄弱点分析使用加权评分：
...
- **LLM 调用**：通过 `api/proxy.js` 代理，前端不直接调用外部 API

## 安全注意事项

- `api/register.js` 中包含硬编码的 admin 密码哈希
...
This project is indexed by GitNexus as **aitutor** (9909 symbols, 11102 relationships, 105 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
