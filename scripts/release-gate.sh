...
step "2/11 前端 contract test (mock)"
CT_RC=0
...
step "3/11 Backend Contract Test (真后端)"
...
step "6/11 仓库一致性 (引用完整性 + D079 边界 + nginx 模板 + 凭据)"
...
# 故在 shell 侧挂接。
step "8/11 api.js 契约闸门 (jsdom, 170 项)"
APIC_RC=0
...
step "9/11 SSE 断连检测 (流式响应禁 req.on('close'))"
...
step "10/11 GraphRAG settings.yaml 字段名 (禁 extra=allow 静默吞字段)"
...
step "11/11 错误回显收口 (err.message 禁入响应体)"
