#!/usr/bin/env node
/**
 * Phase 6 B2 · CH-2 prompt 准备 (复用 33b 模板, 排除 b1 102 题)
 * 生成 database/b2_prompts.jsonl (~6117 题)
 */
...
const OUT = path.join(ROOT, 'database/b2_prompts.jsonl');
const K = 30;
const MAX_STEM = 800;
...
console.log('\n=== B2 准备 prompt (排除 B1 102 题) ===');
const samples = (await c.query(`
  SELECT eq.id, eq.question_uid, eq.subject_code, eq.question_type, eq.year, eq.province_code,
         eq.stem, eq.options::text AS opts
    FROM public.exam_questions eq
   WHERE eq.stem IS NOT NULL AND LENGTH(eq.stem) >= 10
     AND eq.question_uid NOT IN (SELECT question_uid FROM public.b1_sample_seed)
   ORDER BY eq.id
`)).rows;
console.log(`  候选题数: ${samples.length} (排除 B1 102 题后)`);
...
for (const s of Object.keys(v2BySubject)) console.log(`  ${s.padEnd(11)} v2 候选 ${v2BySubject[s].length} 条`);
...
- 选 1–3 个；英语题至少 1 个 ability（如果候选里有）`;
  ...
  out.write(JSON.stringify({
    ...
    stem_chars: stem.length,
    stem_truncated: stemFull.length > MAX_STEM,
    candidate_count: scored.length,
    system, user,
  }) + '\n');
  cnt++;
  if (cnt % 500 === 0) console.log(`  已写 ${cnt}/${samples.length}`);
}
out.end();
await new Promise(r => out.on('finish', r));
console.log(`\n  ✅ 写出: ${path.relative(ROOT, OUT)} (${cnt} 题)`);
