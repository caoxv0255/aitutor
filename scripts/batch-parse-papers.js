...
async function getKPList(pool, subject) {
  try {
    const res = await pool.query(
      'SELECT name FROM knowledge_points WHERE subject_code = $1 LIMIT 100',
      [subject]
    );
...
async function parseSinglePaper(pool, subject, year, filePath) {
...
function findPaperFile(subjectCn, year) {
  ...
  const patterns = [
    `${yearStr}高考北京卷${subjectCn}（原卷版）`,
    `${yearStr}年北京高考${subjectCn}试卷（原卷版）`,
    `${yearStr}北京高考${subjectCn}试题（原卷版）`,
    `${yearStr}年北京高考${subjectCn}试卷`,
    `${yearStr}高考北京卷${subjectCn}`
  ];
...
async function main() {
  console.log('============================================================');
  console.log('📦 批量解析北京高考试卷 (2021-2025)');
  console.log('============================================================');
  ...
  for (const [cnName, code] of Object.entries(SUBJECT_MAP)) {
    ...
    for (const year of YEAR_RANGE) {
      ...
      if (!filePath) {
        console.log(`   ❌ ${year}年: 文件未找到`);
        results.push({ subject: cnName, year, success: false, error: '文件未找到' });
        failCount++;
        continue;
      }
      
      const result = await parseSinglePaper(pool, code, year, filePath);
      results.push({ subject: cnName, year, ...result });
  ...
  for (const [cnName] of Object.entries(SUBJECT_MAP)) {
    const subjectResults = results.filter(r => r.subject === cnName);
    const success = subjectResults.filter(r => r.success).length;
    const total = subjectResults.length;
    console.log(`\n${cnName}: ${success}/${total} 成功`);
    
    subjectResults.forEach(r => {
      if (r.success) {
        console.log(`   ✓ ${r.year}年: ${r.count}道题目`);
      } else {
        console.log(`   ✗ ${r.year}年: ${r.error}`);
      }
    });
  }
