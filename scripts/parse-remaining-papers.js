...
const REMAINING_TASKS = [
  ...
  { subject: 'biology', year: 2025 },
  { subject: 'chemistry', year: 2023 },
  { subject: 'chemistry', year: 2024 },
  { subject: 'chemistry', year: 2025 },
  { subject: 'geography', year: 2024 },
  { subject: 'geography', year: 2025 },
  { subject: 'history', year: 2025 },
  { subject: 'politics', year: 2025 },
  { subject: 'math', year: 2025 }
];
...
function findPaperFile(subjectCode, year) {
  const cnName = Object.keys(SUBJECT_MAP).find(k => SUBJECT_MAP[k] === subjectCode);
  ...
  const patterns = [
    `${yearStr}高考北京卷${cnName}（原卷版）`,
    `${yearStr}年北京高考${cnName}试卷（原卷版）`,
    `${yearStr}北京高考${cnName}试题（原卷版）`,
    `${yearStr}年北京高考${cnName}试卷`,
    `${yearStr}高考北京卷${cnName}`
  ];
  
  for (const pattern of patterns) {
    const found = files.find(f => f.includes(pattern) && f.endsWith('.docx'));
    if (found) {
      return path.join(dirPath, found);
    }
  }
  
  const allYearFiles = files.filter(f => f.includes(yearStr) && f.endsWith('.docx'));
  if (allYearFiles.length > 0) {
    return path.join(dirPath, allYearFiles[0]);
  }
...
async function parseSinglePaper(pool, subject, year) {
  ...
  console.log(`\n📋 解析：${SUBJECT_CN[subject]} ${year}年`);
  
  const filePath = findPaperFile(subject, year);
...
async function main() {
  console.log('============================================================');
  console.log('📦 解析剩余北京高考试卷');
  console.log('============================================================');
  
  const dbUrl = env.PAPERS_DB_URL || env.DATABASE_URL;
  if (!dbUrl) {
    throw new Error('缺少数据库连接串: 请设置 PAPERS_DB_URL（或 DATABASE_URL）。2026-09-21 已移除此处硬编码的 postgres 口令 —— 该口令对现有实例无效, 且已随版本历史暴露。');
  }
  ...
  for (const task of REMAINING_TASKS) {
    const result = await parseSinglePaper(pool, task.subject, task.year);
  ...
  results.forEach(r => {
    if (r.success) {
      console.log(`✓ ${SUBJECT_CN[r.subject]} ${r.year}年: ${r.count}道题目`);
    } else {
      console.log(`✗ ${SUBJECT_CN[r.subject]} ${r.year}年: ${r.error}`);
    }
  });
